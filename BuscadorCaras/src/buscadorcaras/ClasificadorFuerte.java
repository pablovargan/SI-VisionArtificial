...
public class ClasificadorFuerte {
    ...
    // Metodo de Boosting que realiza el algoritmo
            ...
            int numIteraciones, int[] minimos, int[] maximos)
    {
        ...
        // - Recorro lista para asignar el peso
        for(Cara c: listaAprendizaje)
            c.setPeso((double) 1.0/listaAprendizaje.size());
        // Empiezo a buscar-entrenar los clasificadores debiles para crear un clasificador fuerte
        for(int i = 0; i < numIteraciones; i++)
        {
            ...
            // 4. Actualizar el clasificador fuerte y finalmente me quedo con el que mejor
            int aciertos = 0;
            for(Cara cara: listaAprendizaje)
            {
                ...
                // Si son iguales, es un acierto y voy mejorando el clasificador
                if(pos == tipoCara)
                    aciertos++;
            }
